...
import sdarty.larpo.blaol.databinding.TiggyScrnBinding
import kotlin.random.Random

class GameActivityTiggy : AppCompatActivity() {
    private val binding by lazy { TiggyScrnBinding.inflate(layoutInflater) }
    ...
    private fun startGame() {
        ...
        gameCoroutine = CoroutineScope(Dispatchers.Main).launch{
            ...
            for (i in 0..24 ){
                flipKartunka(this@GameActivityTiggy,backCrystals[i], frontCrystals[i])
                delay(100)
            }
            ...
            for (i in 0..24 ){
                flipKartunka(this@GameActivityTiggy,frontCrystals[i], backCrystals[i])
                delay(50)
            }
            delay(500)
